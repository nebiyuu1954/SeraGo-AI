...
# ── Sentence-transformer model (warm at startup, shared by all requests) ─
_model = None
...
def _load_model():
    """Load the model into the global. Uses the local Hugging Face cache when
    the files already exist (skips the slow remote change-checking HEAD
    requests); falls back to a network download on the very first run."""
    global _model
    ...
    try:
        ...
        logger.info("Loaded cached sentence-transformer model: %s", model_name)
    except Exception:
        # Files not cached yet — download from Hugging Face once.
        _model = SentenceTransformer(model_name)
        logger.info("Downloaded + loaded sentence-transformer model: %s", model_name)
...
def warm_model():
    """Start loading the model in the background so the first scoring request
    doesn't pay the load cost. Safe to call repeatedly (server restarts,
    autoreloads). A request that arrives mid-load simply waits on the lock."""
    global _warm_started
    if _model is not None:
        return
    with _model_lock:
        if _model is not None or _warm_started:
            return
        _warm_started = True
    ...
    def _warm():
        try:
            ...
            logger.exception("Model warm-up failed — will retry on next request")
            global _warm_started
            with _model_lock:
                _warm_started = False

    threading.Thread(target=_warm, name="model-warm", daemon=True).start()
...
def encode_texts(texts: list[str]) -> list[list[float]]:
    """Encode many texts in one batched model call (much faster than calling
    encode_text per text — the model vectorizes the whole batch)."""
    if not texts:
        return []
    model = get_model()
